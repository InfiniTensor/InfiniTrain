...
void TestIdentityLambda() {
    ...
    auto sched = LRScheduler::Create<LambdaLR>(opt, [](int64_t) { return 1.0f; });
...
void TestLinearDecayLambda() {
    auto opt = MakeDummyOptimizer(kBaseLR);
    auto sched = LRScheduler::Create<LambdaLR>(opt, [](int64_t step) { return 1.0f - step * 0.1f; });
...
void TestPyTorchAlignment() {
    ...
    auto opt = MakeDummyOptimizer(kBaseLR);
    auto sched = LRScheduler::Create<LambdaLR>(opt, [](int64_t step) {
        return static_cast<float>(std::pow(0.95, step));
    });
...
void TestStateRoundTrip() {
    ...
    auto opt = MakeDummyOptimizer(kBaseLR);
    auto sched = LRScheduler::Create<LambdaLR>(opt, lambda_fn);
    for (int i = 0; i < 5; ++i) sched->Step();
    StateDict saved = sched->State();

    auto opt2 = MakeDummyOptimizer(kBaseLR);
    auto sched2 = LRScheduler::Create<LambdaLR>(opt2, lambda_fn);  // same lambda
...
void TestResumeConsistency() {
    ...
    auto sched_ref = LRScheduler::Create<LambdaLR>(opt_ref, lambda_fn);
    for (int i = 0; i < kN; ++i) sched_ref->Step();

    auto opt_a = MakeDummyOptimizer(kBaseLR);
    auto sched_a = LRScheduler::Create<LambdaLR>(opt_a, lambda_fn);
    for (int i = 0; i < kK; ++i) sched_a->Step();
    StateDict ckpt = sched_a->State();

    auto opt_b = MakeDummyOptimizer(kBaseLR);
    auto sched_b = LRScheduler::Create<LambdaLR>(opt_b, lambda_fn);
