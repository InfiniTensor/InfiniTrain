...
void TestIdentityLambda() {
    ...
    LambdaLR sched(opt, [](int64_t) { return 1.0f; });
...
void TestLinearDecayLambda() {
    auto opt = MakeDummyOptimizer(kBaseLR);
    LambdaLR sched(opt, [](int64_t step) { return 1.0f - step * 0.1f; });
...
void TestPyTorchAlignment() {
    ...
    auto opt = MakeDummyOptimizer(kBaseLR);
    LambdaLR sched(opt, [](int64_t step) {
