...
void TestMultiplicativeChain() {
    ...
    auto lambda_lr = LRScheduler::Create<LambdaLR>(opt,
        [](int64_t step) { return 1.0f - 0.1f * step; });

    auto sched = LRScheduler::Create<ChainedScheduler>(opt, std::vector<std::shared_ptr<LRScheduler>>{step_lr, lambda_lr});
...
void TestConstantPlusStep() {
    ...
    auto sched = LRScheduler::Create<ChainedScheduler>(opt, std::vector<std::shared_ptr<LRScheduler>>{constant, step_lr});
...
void TestConstantPlusStepDLC() {
    ...
    auto step_lr = LRScheduler::Create<StepLR>(opt, /*step_size=*/3, /*gamma=*/0.1f);
    auto Lambda = LRScheduler::Create<LambdaLR>(opt,
        [](int64_t step) { return 1.0f - 0.1f * step; });
...
void TestStateRoundTrip() {
    ...
    auto lambda_lr = std::make_shared<LambdaLR>(opt,
        [](int64_t step) { return 1.0f - 0.05f * step; });
    auto sched = LRScheduler::Create<ChainedScheduler>(opt, std::vector<std::shared_ptr<LRScheduler>>{step_lr, lambda_lr});
    ...
    auto lambda_lr2 = std::make_shared<LambdaLR>(opt2,
        [](int64_t step) { return 1.0f - 0.05f * step; });
    auto sched2 = LRScheduler::Create<ChainedScheduler>(opt2, std::vector<std::shared_ptr<LRScheduler>>{step_lr2, lambda_lr2});
...
void TestResumeConsistency() {
    ...
    auto make_sched = [&](std::shared_ptr<Optimizer> opt) {
        ...
        return LRScheduler::Create<ChainedScheduler>(opt, std::vector<std::shared_ptr<LRScheduler>>{step_lr, lambda_lr});
    };
