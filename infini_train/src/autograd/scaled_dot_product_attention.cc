...
ScaledDotProductAttention::Backward(const std::vector<std::shared_ptr<Tensor>> &grad_outputs) {
    ...
    auto [grad_query, grad_key, grad_value]
        = Dispatcher::Instance().Call<std::tuple<std::shared_ptr<Tensor>, std::shared_ptr<Tensor>, std::shared_ptr<Tensor>>>(
            {device, "ScaledDotProductAttentionBackward"}, query, key, value, output, stats, grad_output, attn_scale,
            is_causal_);
