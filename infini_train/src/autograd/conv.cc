...
std::vector<std::shared_ptr<Tensor>> Conv2d::Forward(const std::vector<std::shared_ptr<Tensor>> &input_tensors) {
    ...
    return {Dispatcher::Instance().Call<std::shared_ptr<Tensor>>({device, "Conv2dForward"}, input, weight, bias,
                                                                 stride_, padding_)};
}
...
std::vector<std::shared_ptr<Tensor>> Conv2d::Backward(const std::vector<std::shared_ptr<Tensor>> &grad_outputs) {
    ...
    bool need_grad_bias = ctx_.needs_input_grad().size() > 2 && ctx_.needs_input_grad()[2];
    ...
    if (need_grad_input) {
        CHECK_NE(weight, nullptr) << "Conv2d::Backward requires the saved weight to compute grad_input";
        grad_input = Dispatcher::Instance().Call<std::shared_ptr<Tensor>>({device, "Conv2dBackwardInput"}, weight,
                                                                          grad_output, stride_, padding_, input_dims_);
    }
    if (need_grad_weight) {
        CHECK_NE(input, nullptr) << "Conv2d::Backward requires the saved input to compute grad_weight";
        grad_weight = Dispatcher::Instance().Call<std::shared_ptr<Tensor>>(
            {device, "Conv2dBackwardWeight"}, input, grad_output, stride_, padding_, weight_dims_);
    }
    if (need_grad_bias) {
