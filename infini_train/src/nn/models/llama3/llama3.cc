...
//     auto q_rotated
//         = nn::function::Stack(std::vector<std::shared_ptr<Tensor>>{q_rotated_left, q_rotated_right},
//         -1)->Flatten(-2);
...
//     auto k_rotated
//         = nn::function::Stack(std::vector<std::shared_ptr<Tensor>>{k_rotated_left, k_rotated_right},
//         -1)->Flatten(-2);
...
//     // v: (B, T, KV_local, D)
//     auto v = qkv->Slice(2, q_size_local + kv_size_local, q_size_local + 2 * kv_size_local)->View({B, T, KV_local,
//     D});
