...
[[noreturn]] void Fail(const std::string &msg) { throw PipelineLayoutError(msg); }

void CheckStageId(int stage_id, int num_stages, const std::string &field) {
    if (stage_id < 0 || stage_id >= num_stages) {
        Fail(std::format("{}={} is outside valid stage range [0, {})", field, stage_id, num_stages));
    }
}
...
bool LayerRange::contains(int layer_id) const { return layer_id < end && layer_id >= start; }

PipelineLayout::PipelineLayout() : num_layers_(0), num_stages_(1), vpp_size_(1) { BuildIndexes(); }

PipelineLayout PipelineLayout::BuildDefault(int num_layers, int num_stages, int vpp_size,
    ...
    const long long total_chunks_ll = static_cast<long long>(num_stages) * static_cast<long long>(vpp_size);
...
PipelineLayout PipelineLayout::BuildContiguous(const std::vector<int> &stage_layer_counts,
                                               SpecialModulePlacement placement, PipelineLayoutPolicy policy) {
...
PipelineLayout::PipelineLayout(int num_layers, int num_stages, int vpp_size, std::vector<ChunkLayout> chunks,
                               SpecialModulePlacement placement, PipelineLayoutPolicy policy)
    : num_layers_(num_layers), num_stages_(num_stages), vpp_size_(vpp_size), chunks_(std::move(chunks)),
      placement_(std::move(placement)), policy_(std::move(policy)) {
    Validate();
    BuildIndexes();
}
...
void PipelineLayout::BuildIndexes() {
    ...
    for (int i = 0; i < num_stages_; ++i) { stages_[i].stage_id = i; }
    for (const auto &c : chunks_) { stages_[c.stage_id].global_chunk_ids.push_back(c.global_chunk_id); }
}
...
int PipelineLayout::stage_of_layer(int layer_id) const { return locate_layer(layer_id).stage_id; }
...
int PipelineLayout::local_layer_index(int stage_id, int layer_id) const {
    ...
    if (loc.stage_id != stage_id) {
        Fail(std::format("layer {} belongs to stage {}, not {}", layer_id, loc.stage_id, stage_id));
    }
...
void PipelineLayout::Validate() const {
    ...
    for (const auto &c : chunks_) {
        if (c.layers.start < 0 || c.layers.end < c.layers.start || c.layers.end > num_layers_) {
            Fail(std::format("chunk {} layer range [{}, {}) is outside [0, {})", c.global_chunk_id, c.layers.start,
                             c.layers.end, num_layers_));
        }
    ...
    for (const auto &c : chunks_) {
        if (c.stage_id < 0 || c.stage_id >= num_stages_) {
            Fail(std::format("chunk {} stage_id={} out of range [0, {})", c.global_chunk_id, c.stage_id, num_stages_));
        }
    ...
    for (const auto &c : chunks_) {
        ...
        if (c.local_chunk_id < 0) {
            Fail(std::format("chunk {} has negative local_chunk_id={}", c.global_chunk_id, c.local_chunk_id));
        }
    }
    ...
    if (policy_.require_boundary_special_modules) {
        if (placement_.embedding_stage != 0) {
            Fail(std::format("embedding_stage={} must be boundary stage 0", placement_.embedding_stage));
        }
        if (placement_.final_norm_stage != num_stages_ - 1) {
            Fail(
                std::format("final_norm_stage={} must be last stage {}", placement_.final_norm_stage, num_stages_ - 1));
        }
        if (placement_.lm_head_stage != num_stages_ - 1) {
            Fail(std::format("lm_head_stage={} must be last stage {}", placement_.lm_head_stage, num_stages_ - 1));
        }
    }
    ...
    if (!policy_.allow_empty_stages) {
        ...
        for (int i = 0; i < num_stages_; ++i) {
            if (!stage_has_layers[i]) {
                Fail(std::format("stage {} has no layers; set allow_empty_stages=true to permit", i));
            }
        }
    }
    ...
    if (policy_.require_contiguous_execution) {
        ...
        std::sort(sorted.begin(), sorted.end(),
                  [](const ChunkLayout &a, const ChunkLayout &b) { return a.layers.start < b.layers.start; });
        for (size_t i = 1; i < sorted.size(); ++i) {
            if (sorted[i].layers.start != sorted[i - 1].layers.end) {
                Fail(std::format("layer ranges are not contiguous: gap between [{}, {}) and [{}, {})",
                                 sorted[i - 1].layers.start, sorted[i - 1].layers.end, sorted[i].layers.start,
                                 sorted[i].layers.end));
            }
        }
    }
...
void PipelineLayout::ValidateForCurrentPipelineTransport() const {
    ...
    for (int i = 0; i < num_stages_; ++i) {
        if (stages_[i].global_chunk_ids.empty()) {
            Fail(std::format("stage {} has no chunks; the current pipeline transport requires "
                             "every stage to have at least one chunk",
                             i));
        }
    }
}
...
std::string PipelineLayout::ToString() const {
    ...
    out << std::format("  special_modules: embedding={}, final_norm={}, lm_head={}\n", placement_.embedding_stage,
                       placement_.final_norm_stage, placement_.lm_head_stage);
    ...
    for (int sid = 0; sid < num_stages_; ++sid) {
        ...
        if (owns(SpecialModule::kEmbedding, sid)) {
            specials.push_back("embedding");
        }
        if (owns(SpecialModule::kFinalNorm, sid)) {
            specials.push_back("final_norm");
        }
        if (owns(SpecialModule::kLMHead, sid)) {
            specials.push_back("lm_head");
        }
        if (!specials.empty()) {
            out << " {";
...
PipelineLayout PipelineLayout::ParseMegatronStyleLayout(const std::string &value, int num_layers, int pp_size,
                                                        int vpp_size, SpecialModulePlacement placement,
                                                        PipelineLayoutPolicy policy) {
    if (value.empty()) {
        Fail("pipeline_layout must not be empty");
    }
    if (num_layers <= 0 || pp_size <= 0 || vpp_size <= 0) {
        Fail(std::format("num_layers, pp_size and vpp_size must be positive (got {}, {}, {})", num_layers, pp_size,
                         vpp_size));
    }
    ...
    std::function<std::string(std::string_view)> expand = [&](std::string_view input) {
        ...
        for (size_t i = 0; i < input.size();) {
            ...
            if (depth != 0) {
                Fail("pipeline_layout has unmatched '('");
            }
            ...
            if (j < input.size() && input[j] == '*') {
                ...
                while (k < input.size() && input[k] >= '0' && input[k] <= '9') {
                    ...
                    if (repeat > 100000) {
                        Fail("pipeline_layout repetition is too large");
                    }
                }
                if (repeat == 0) {
                    Fail("pipeline_layout repetition must be positive");
                }
                j = k;
            }
    ...
    while (true) {
        ...
        stage_exprs.emplace_back(expanded.substr(begin, sep == std::string::npos ? std::string::npos : sep - begin));
    ...
    if (static_cast<int>(stage_exprs.size()) != pp_size) {
        Fail(std::format("pipeline_layout stage count mismatch: pp_size={}, stages={}", pp_size, stage_exprs.size()));
    }
    ...
    for (int stage_id = 0; stage_id < pp_size; ++stage_id) {
        ...
        while (true) {
            ...
            const std::string token
                = expr.substr(chunk_begin, comma == std::string::npos ? std::string::npos : comma - chunk_begin);
            int t_count = 0;
            for (char c : token) {
                switch (c) {
                case 't':
                ...
                case 'E':
                    if (saw_embedding) {
                        Fail("pipeline_layout contains multiple E tokens");
                    }
                    placement.embedding_stage = stage_id;
                    saw_embedding = true;
                    break;
                case 'F':
                    if (saw_final_norm) {
                        Fail("pipeline_layout contains multiple F tokens");
                    }
                    placement.final_norm_stage = stage_id;
                    saw_final_norm = true;
                    break;
                case 'H':
                case 'L':
                    if (saw_lm_head) {
                        Fail("pipeline_layout contains multiple H/L tokens");
                    }
                    placement.lm_head_stage = stage_id;
                    saw_lm_head = true;
                    break;
                case ' ':
                case '\t':
        ...
        if (static_cast<int>(layer_counts[stage_id].size()) != vpp_size) {
            Fail(std::format("pipeline_layout chunk count mismatch at stage {}: vpp_size={}, chunks={}", stage_id,
                             vpp_size, layer_counts[stage_id].size()));
        }
    }
    ...
    if (layer_cursor != num_layers) {
        Fail(std::format("pipeline_layout layer count mismatch: num_layers={}, parsed_layers={}", num_layers,
                         layer_cursor));
    }
    if (!saw_embedding) {
        placement.embedding_stage = 0;
    ...
    if (!saw_final_norm) {
        placement.final_norm_stage = pp_size - 1;
    }
    if (!saw_lm_head) {
        placement.lm_head_stage = pp_size - 1;
    }
    ...
    return PipelineLayout(num_layers, pp_size, vpp_size, std::move(chunks), BuildPlacement(placement, pp_size), policy);
}

std::vector<int> PipelineLayout::SuggestBalancedPartition(int num_layers, int pp_size,
                                                          const std::vector<double> &layer_costs) {
    if (num_layers <= 0 || pp_size <= 0) {
        Fail(std::format("num_layers and pp_size must be positive (got {}, {})", num_layers, pp_size));
    }
    ...
    if (costs.empty()) {
        costs.assign(static_cast<size_t>(num_layers), 1.0);
    }
    if (static_cast<int>(costs.size()) != num_layers) {
        Fail(std::format("layer_costs length mismatch: num_layers={}, costs={}", num_layers, costs.size()));
    }
    ...
    for (int stage = 0; stage < pp_size - 1; ++stage) {
        ...
        while (take < layers_left - (stages_left - 1) && accum + costs[cursor + take] <= target) {
        ...
        if (take < layers_left - (stages_left - 1)
            && std::abs(accum + costs[cursor + take] - target) < std::abs(accum - target)) {
            accum += costs[cursor + take++];
        }
...
PipelineLayout PipelineLayout::BuildPipelineLayout(int num_layers, int pp_size, int vpp_size,
    ...
    if (static_cast<int>(pp.size()) != pp_size) {
        Fail(std::format("stage count mismatch: pipeline_parallel={}, partition_entries={}", pp_size, pp.size()));
    }

    long long layer_sum = 0;
    for (int layer_count : pp) { layer_sum += layer_count; }

    if (layer_sum != num_layers) {
        Fail(std::format("layer sum mismatch: num_layers={}, partition_sum={}", num_layers, layer_sum));
    }

    if (vpp_size != 1) {
        Fail(std::format("custom layer partition is not supported with vpp_size={}", vpp_size));
    }
