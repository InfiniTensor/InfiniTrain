...
std::vector<std::pair<std::string, std::shared_ptr<Module>>> Module::named_modules(bool recurse,
                                                                                   bool remove_duplicate) const {
    if (!recurse) {
        return {{"", const_cast<Module *>(this)->shared_from_this()}};
    }
    return const_cast<Module *>(this)->NamedModules("", remove_duplicate, nullptr);
}
...
Module::NamedModules(const std::string &prefix, bool remove_duplicate, std::unordered_set<Module *> *memory) {
    ...
    for (const auto &[name, module] : children) {
        ...
        auto sub = module->NamedModules(submodule_prefix, remove_duplicate, memory);
