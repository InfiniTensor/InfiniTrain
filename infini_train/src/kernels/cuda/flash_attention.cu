...
std::vector<std::shared_ptr<Tensor>> FlashAttentionBackward(const std::shared_ptr<Tensor> &q,
                                                             ...
                                                             const std::shared_ptr<Tensor> &do_,
                                                             bool is_causal) {
    CHECK(q->Dtype() == DataType::kBFLOAT16);
    ...
    auto do_f32 = do_->Dtype() == DataType::kFLOAT32
                      ? do_
                      : std::make_shared<Tensor>(do_->To(DataType::kFLOAT32));
    ...
    attention_v6_backward(static_cast<const nv_bfloat16 *>(q->DataPtr()),
                          ...
                          static_cast<const float *>(l->DataPtr()),
                          static_cast<const float *>(do_f32->DataPtr()),
                          static_cast<float *>(dq_f32->DataPtr()),
                          static_cast<float *>(dk_f32->DataPtr()),
                          static_cast<float *>(dv_f32->DataPtr()),
                          static_cast<int>(B),
                          static_cast<int>(q_head),
                          static_cast<int>(kv_head),
                          static_cast<int>(q_len),
