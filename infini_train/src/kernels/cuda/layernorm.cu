...
LayerNormForward(const std::shared_ptr<Tensor> &input, const std::shared_ptr<Tensor> &weight,
    ...
    switch (input->Dtype()) {
        DISPATCH_CASE(
            DataType::kFLOAT32,
            WRAP(LayerNormForwardKernel<BLOCK_SIZE><<<num_blocks, threads_per_block, shared_mem_size>>>(
...
LayerNormBackward(const std::shared_ptr<Tensor> &input, const std::shared_ptr<Tensor> &weight,
    ...
    switch (input->Dtype()) {
        DISPATCH_CASE(
            DataType::kFLOAT32,
            WRAP(LayerNormBackwardKernel<BLOCK_SIZE><<<num_blocks, threads_per_block>>>(
