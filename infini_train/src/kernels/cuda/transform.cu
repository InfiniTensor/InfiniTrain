...
std::shared_ptr<Tensor> TrilForward(const std::shared_ptr<Tensor> &input, int64_t diagonal) {
    ...
    DispatchFunc<INFINI_ALL_TYPES>(
        input->Dtype(),
        [=]<typename T>() {
            TrilForwardKernel<<<num_blocks, threads_per_block>>>(
...
std::shared_ptr<Tensor> TrilBackward(const std::shared_ptr<Tensor> &grad_output, int64_t diagonal) {
    ...
    DispatchFunc<INFINI_ALL_TYPES>(
        ...
        [=]<typename T>() {
            grad_input->Fill<T>(0);
            TrilBackwardKernel<<<num_blocks, threads_per_block>>>(static_cast<const T *>(grad_output->DataPtr()),
                                                                  static_cast<T *>(grad_input->DataPtr()), rows, cols,
                                                                  diagonal);
        },
        "TrilBackward");
...
std::shared_ptr<Tensor> TriuForward(const std::shared_ptr<Tensor> &input, int64_t diagonal) {
    ...
    DispatchFunc<INFINI_ALL_TYPES>(
        input->Dtype(),
        [=]<typename T>() {
            TriuForwardKernel<<<num_blocks, threads_per_block>>>(
...
std::shared_ptr<Tensor> TriuBackward(const std::shared_ptr<Tensor> &grad_output, int64_t diagonal) {
    ...
    DispatchFunc<INFINI_ALL_TYPES>(
        ...
        [=]<typename T>() {
            grad_input->Fill<T>(0);
            TriuBackwardKernel<<<num_blocks, threads_per_block>>>(static_cast<const T *>(grad_output->DataPtr()),
                                                                  static_cast<T *>(grad_input->DataPtr()), rows, cols,
                                                                  diagonal);
        },
        "CUDA TriuBackward");
...
std::shared_ptr<Tensor> RepeatInterleaveForward(const std::shared_ptr<Tensor> &input, int64_t repeat, int64_t dim) {
    ...
    DispatchFunc<INFINI_ALL_TYPES>(
        input->Dtype(),
        [=]<typename T>() {
            RepeatInterleaveForwardKernel<<<num_blocks, threads_per_block>>>(static_cast<const T *>(input->DataPtr()),
                                                                             static_cast<T *>(output->DataPtr()), outer,
...
std::shared_ptr<Tensor> RepeatInterleaveBackward(const std::shared_ptr<Tensor> &grad_output,
    ...
    DispatchFunc<INFINI_ALL_TYPES>(
        ...
        [=]<typename T>() {
            grad_input->Fill<T>(0);
            RepeatInterleaveBackwardKernel<<<num_blocks, threads_per_block>>>(
