...
std::shared_ptr<Tensor> StackForward(const std::vector<std::shared_ptr<Tensor>> &inputs, int64_t dim) {
    ...
    DispatchFunc<INFINI_ALL_TYPES>(
        ...
        [=]<typename T>() {
            ...
            cudaMallocAsync(&device_input_ptrs, sizeof(T *) * num_inputs, 0);
            cudaMemcpyAsync(device_input_ptrs, host_input_ptrs.data(), sizeof(T *) * num_inputs, cudaMemcpyHostToDevice,
                            0);

            StackForwardKernel<<<num_blocks, threads_per_block>>>(
...
std::vector<std::shared_ptr<Tensor>> StackBackward(const std::vector<int64_t> &input_dims, int64_t dim,
    ...
    DispatchFunc<INFINI_ALL_TYPES>(
        ...
        [=]<typename T>() {
            ...
            T **device_ptrs;
            cudaMallocAsync(&device_ptrs, sizeof(T *) * num_inputs, 0);
            cudaMemcpyAsync(device_ptrs, host_ptrs.data(), sizeof(T *) * num_inputs, cudaMemcpyHostToDevice, 0);

            StackBackwardKernel<<<num_blocks, threads_per_block>>>(static_cast<const T *>(grad_output->DataPtr()),
