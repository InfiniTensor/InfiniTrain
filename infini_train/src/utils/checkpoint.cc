...
struct CheckpointFrame {
    ...
    void Recompute(int64_t gid) {
        ...
        AutocastStateGuard autocast_guard(autocast_state);
        ...
        autograd::Function::SavedTensorHooks recompute_hooks;
        ...
        autograd::Function::SavedTensorHooksGuard guard(std::move(recompute_hooks));
...
CheckpointFunction::Backward(const std::vector<std::shared_ptr<Tensor>> &grad_outputs) {
    ...
    AutocastStateGuard autocast_guard(saved_autocast_);
...
std::vector<std::shared_ptr<Tensor>> Checkpoint(const CheckpointForwardFn &forward_fn,
    ...
    autograd::Function::SavedTensorHooks placeholder_hooks;
    ...
    autograd::Function::SavedTensorHooksGuard guard(std::move(placeholder_hooks));
    return forward_fn(inputs);
}
