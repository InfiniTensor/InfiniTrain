...
public:
    ...
    std::string Finalize() {
        ...
        for (int i = 0; i < 4; ++i) {
            oss << std::setw(2) << ((state_[0] >> (i * 8)) & 0xff);
        }
        for (int i = 0; i < 4; ++i) {
            oss << std::setw(2) << ((state_[1] >> (i * 8)) & 0xff);
        }
        for (int i = 0; i < 4; ++i) {
            oss << std::setw(2) << ((state_[2] >> (i * 8)) & 0xff);
        }
        for (int i = 0; i < 4; ++i) {
            oss << std::setw(2) << ((state_[3] >> (i * 8)) & 0xff);
        }
        return oss.str();
    }
...
void PrintTableRow(std::ostream& os, const std::string& key, int level, const std::string& shape,
    ...
    os << "| " << std::left << std::setw(49) << key.substr(0, 49) << "| " << std::setw(6) << level << "| "
       << std::setw(17) << shape.substr(0, 17) << "| " << std::setw(14) << dtype << "| " << std::setw(9)
       << same_hash << "| " << std::setw(9) << diff_order << "|\n";
}
...
std::string CalculateDiffOrder(const float* data1, const float* data2, size_t size) {
    if (!data1 || !data2 || size == 0) return "N/A";
        ...
        if (global_config.format == "table") {
            ...
            if (has_error || config.print_stats) {
                ...
                if (global_config.output_md5) {
                    log_stream << "md5=" << md5;
                    if (!same_hash) log_stream << " (MISMATCH)";
                } else {
                    log_stream << "[";
                    ...
                    if (config.print_stats) {
                        ...
                        if (num_elements > max_print) log_stream << ", ...";
                    }
                    log_stream << "]";
                }
                log_stream << std::endl;
...
void PrecisionChecker::RegisterForFunction(autograd::Function* func, const std::string& name, const Config& config) {
    ...
    func->RegisterForwardPostHook([func_name, config](autograd::Function*, const std::vector<std::shared_ptr<Tensor>>&,
