...
MLP::MLP(const TransformerConfig &config, const ModuleSpec &spec) : CloneableModule(kType) {
    ...
    CHECK(spec.submodules_.contains(kCFcLayerName)) << "MLP spec missing submodule: " << kCFcLayerName;
    ...
    if (spec.submodules_.count(kCFc2LayerName) > 0) {
        CHECK(spec.submodules_.contains(kCFc2LayerName)) << "MLP spec missing submodule: " << kCFc2LayerName;
        modules_[kCFc2LayerName] = build_module(config, spec.submodules_.at(kCFc2LayerName));
    }

    // Activation: check for GELU or SwiGLU
    if (spec.submodules_.count(kGeluLayerName) > 0) {
        modules_[kGeluLayerName] = build_module(config, spec.submodules_.at(kGeluLayerName));
    } else if (spec.submodules_.count(kSiluLayerName) > 0) {
        CHECK(spec.submodules_.contains(kSiluLayerName)) << "MLP spec missing submodule: " << kSiluLayerName;
    ...
    CHECK(spec.submodules_.contains(kCProjLayerName)) << "MLP spec missing submodule: " << kCProjLayerName;
    modules_[kCProjLayerName] = build_module(config, spec.submodules_.at(kCProjLayerName));
}
...
REGISTER_MODULE_CUSTOM(LayerNorm, [](const TransformerConfig &config, const ModuleSpec &spec) {
    auto normalized_shape
        = GetOptionalParam<std::vector<int64_t>>(spec, "normalized_shape", std::vector<int64_t>{config.n_embd});
...
REGISTER_MODULE_CUSTOM(RMSNorm, [](const TransformerConfig &config, const ModuleSpec &spec) {
    int64_t dim = GetOptionalParam<int64_t>(spec, "dim", config.n_embd);
    float eps = GetOptionalParam<float>(spec, "eps", 1e-5f);
...
REGISTER_MODULE_CUSTOM(Embedding, [](const TransformerConfig &config, const ModuleSpec &spec) {
    int num_embeddings = GetRequiredParam<int>(spec, "num_embeddings");
    int embedding_dim = GetRequiredParam<int>(spec, "embedding_dim");
...
REGISTER_MODULE_CUSTOM(ColumnParallelLinear, [](const TransformerConfig &config, const ModuleSpec &spec) {
    int in = GetRequiredParam<int>(spec, "in");
    int out = GetRequiredParam<int>(spec, "out");
    bool bias = GetOptionalParam<bool>(spec, "bias", true);
