...
TransformerModel::TransformerModel(const TransformerConfig config, const ModuleSpec &spec
    ...
    if (stage_info_.is_first_stage) {
        modules_[kPPFirstStageName]
            = std::make_shared<TransformerFirstStage>(config_, spec_.submodules_.at(TransformerFirstStage::kType));
    ...
    if (stage_info_.is_last_stage) {
        modules_[kPPLastStageName]
            = std::make_shared<TransformerLastStage>(config_, spec_.submodules_.at(TransformerLastStage::kType));
