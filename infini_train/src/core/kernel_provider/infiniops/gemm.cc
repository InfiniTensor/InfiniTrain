...
void Gemm(Device device, kernels::GemmParams p) {
    ...
    infini::ops::functional::Gemm(handle, {}, b, a, std::optional<float>{p.alpha}, std::optional<float>{p.beta},
...
REGISTER_INFINIOPS_KERNEL(Gemm, infini_train::kernel_provider::infiniops::Gemm)
