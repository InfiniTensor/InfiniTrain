...
struct PrecisionCheckConfig {
    ...
    static PrecisionCheckConfig Parse(const std::string& config_str) {
        ...
        if (config_str.empty()) return config;
        ...
        if (kv_map.count("level")) config.level = std::stoi(kv_map["level"]);
        if (kv_map.count("output_path")) config.output_path = kv_map["output_path"];
        if (kv_map.count("output_md5")) {
            config.output_md5 = (kv_map["output_md5"] == "true" || kv_map["output_md5"] == "1");
        }
        if (kv_map.count("format")) config.format = kv_map["format"];
        if (kv_map.count("baseline")) config.baseline_path = kv_map["baseline"];
