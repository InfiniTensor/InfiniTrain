...
template <Device::DeviceType Dev, typename Scalar>
using NativeScalar_t = typename NativeScalar<Dev, Scalar>::type;

// Optional convenience alias for CUDA call sites.
// Keep only one copy here; backend files should NOT redefine it.
template <typename Scalar>
using NativeScalarCUDA_t = NativeScalar_t<Device::DeviceType::kCUDA, Scalar>;
...
struct HasNativeScalar : std::false_type {};
...
template <Device::DeviceType Dev, typename Scalar, typename Enable = void>
struct ScalarConvert;

// Default FP16 conversion: preserve raw 16-bit bit pattern.
template <Device::DeviceType Dev>
struct ScalarConvert<Dev, infini_train::FP16, void> {
...
template <Device::DeviceType Dev>
struct ScalarConvert<Dev, infini_train::BF16, void> {
...
// -----------------------------------------------------------------------------
template <Device::DeviceType Dev, typename Scalar>
inline NativeScalar_t<Dev, Scalar> ToNative(Scalar v) noexcept {
    return ScalarConvert<Dev, Scalar>::ToNative(v);
}

template <Device::DeviceType Dev, typename Scalar>
inline Scalar FromNative(NativeScalar_t<Dev, Scalar> v) noexcept {
