...
TEST_P(AutogradConv2dBackwardTest, Conv2dBackwardStride1NoPadding) {
    auto input = MakeTensor({1, 2, 4, 4}, a_input, GetDevice(), /*requires_grad=*/true);
    auto weight = MakeTensor({3, 2, 3, 3}, a_weight, GetDevice(), /*requires_grad=*/true);
    auto bias = MakeTensor({3}, a_bias, GetDevice(), /*requires_grad=*/true);
    auto grad_output = MakeTensor({1, 3, 2, 2}, a_grad_output, GetDevice());

    auto conv_fn = std::make_shared<autograd::Conv2d>(/*stride=*/1, /*padding=*/0);
    conv_fn->Apply({input, weight, bias});

    const auto grads = conv_fn->Backward({grad_output});
    EXPECT_EQ(grads.size(), 3);
    test::ExpectTensorNear(grads[0], a_grad_input, 1e-4f);
    test::ExpectTensorNear(grads[1], a_grad_weight, 1e-4f);
    test::ExpectTensorNear(grads[2], a_grad_bias, 1e-4f);
}

TEST_P(AutogradConv2dBackwardTest, Conv2dBackwardStride2Padding1Batch) {
    auto input = MakeTensor({2, 1, 5, 5}, b_input, GetDevice(), /*requires_grad=*/true);
    auto weight = MakeTensor({2, 1, 2, 2}, b_weight, GetDevice(), /*requires_grad=*/true);
    auto bias = MakeTensor({2}, b_bias, GetDevice(), /*requires_grad=*/true);
    auto grad_output = MakeTensor({2, 2, 3, 3}, b_grad_output, GetDevice());
    ...
    const auto grads = conv_fn->Backward({grad_output});
    EXPECT_EQ(grads.size(), 3);
    test::ExpectTensorNear(grads[0], b_grad_input, 1e-4f);
    test::ExpectTensorNear(grads[1], b_grad_weight, 1e-4f);
    test::ExpectTensorNear(grads[2], b_grad_bias, 1e-4f);
}

TEST_P(AutogradConv2dBackwardTest, Conv2dBackwardSkipsUnusedInputs) {
    // Without bias and with a non-differentiable input, only the weight gradient is produced.
    auto input = std::make_shared<Tensor>(std::vector<int64_t>{1, 2, 4, 4}, DataType::kFLOAT32, GetDevice(), false);
    input->Fill(0.5f);
    ...
    auto conv_fn = std::make_shared<autograd::Conv2d>(/*stride=*/1, /*padding=*/0);
    conv_fn->Apply({input, weight});

    const auto grads = conv_fn->Backward({grad_output});
    EXPECT_EQ(grads.size(), 3);
    EXPECT_EQ(grads[0], nullptr);
    EXPECT_NE(grads[1], nullptr);
    EXPECT_EQ(grads[2], nullptr);
}
