...
class AutogradConvCudaBackwardTest : public infini_train::test::InfiniTrainTest {};

// Hand-checkable case: x = arange(9) reshaped (1,1,3,3), w = [[1,2],[3,4]], bias = 0.5,
// grad_output = ones(1,1,2,2).
TEST_P(AutogradConvCudaBackwardTest, ConvBackwardGradients) {
    ONLY_CUDA();
...
// No-bias path returns {grad_input, grad_weight} and both must be correct.
TEST_P(AutogradConvCudaBackwardTest, ConvBackwardNoBias) {
    ONLY_CUDA();
...
TEST_P(AutogradConvCudaBackwardTest, ConvBackwardKernelEqualsSpatial) {
    ONLY_CUDA();
...
TEST_P(AutogradConvCudaBackwardTest, ConvBackwardInputNotRequired) {
    ONLY_CUDA();
...
TEST_P(AutogradConvCudaBackwardTest, ConvBackwardTorchGolden) {
    ONLY_CUDA();
...
TEST_P(AutogradConvCudaBackwardTest, ConvBackwardEmptyBatch) {
    ONLY_CUDA();
    auto input
...
INFINI_TRAIN_REGISTER_TEST(AutogradConvCudaBackwardTest);
