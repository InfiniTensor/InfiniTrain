...
namespace {
// Deterministic golden data, generated with torch 2.14 under seed 20260905
// (conv2d / autograd.grad reference).
constexpr float kGoldenAbsError = 2e-6f;
} // namespace

class AutogradConvCudaForwardTest : public infini_train::test::InfiniTrainTest {};

// An asymmetric kernel pins the cross-correlation semantics: flipping the kernel would swap
// the roles of the taps and change every output value. The values are small exact integers,
// so the CUDA path reproduces them bit-exactly.
TEST_P(AutogradConvCudaForwardTest, ConvForwardAsymmetricKernel) {
    ONLY_CUDA();
...
// Cross-channel / batch weighting with a per-channel bias.
TEST_P(AutogradConvCudaForwardTest, ConvForwardMultiChannelBatchBias) {
    ONLY_CUDA();
    std::vector<float> input_values;
    for (int n = 0; n < 2; ++n) {
        for (int c = 0; c < 2; ++c) {
...
// 1x1 kernels reduce conv to a per-position channel mix.
TEST_P(AutogradConvCudaForwardTest, ConvForwardKernelOne) {
    ONLY_CUDA();
...
// H = W = kernel collapses the output to 1x1.
TEST_P(AutogradConvCudaForwardTest, ConvForwardKernelEqualsSpatial) {
    ONLY_CUDA();
    std::vector<float> input_values;
...
// Magnitudes near the fp32 integer-exact range guard against overflow on accumulation.
TEST_P(AutogradConvCudaForwardTest, ConvForwardExtremeValues) {
    ONLY_CUDA();
...
TEST_P(AutogradConvCudaForwardTest, ConvForwardTorchGolden) {
    ONLY_CUDA();
...
TEST_P(AutogradConvCudaForwardTest, ConvForwardEmptyBatch) {
    ONLY_CUDA();
...
INFINI_TRAIN_REGISTER_TEST(AutogradConvCudaForwardTest);
