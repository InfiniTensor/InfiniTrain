# FlashAttention 项目提交报告（按 require.md）
...
```bash
cd scripts
MODEL_EXTRA_ARGS='--flash=false --num_iteration=1' ./run_models_and_profile.bash --continue-on-error
MODEL_EXTRA_ARGS='--flash=true --num_iteration=1' ./run_models_and_profile.bash --continue-on-error
```
