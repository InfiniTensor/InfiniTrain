...
constexpr int32_t kQwen3FP32Version = 3;
} // namespace

namespace qwen3 {

// 待确认 checkpoint格式，暂时参考llama3的格式，后续根据实际情况修改
std::shared_ptr<nn::TransformerModel> LoadFromLLMC(const std::string &filepath) {
    ...
    if (ffn_dim_multiplier > 0.0f) {
        qwen3_config.ffn_dim_multiplier = ffn_dim_multiplier;
    }
    ...
    LOG(INFO) << "DEBUG after model create: ffn_expansion_ratio=" << qwen3_config.ffn_expansion_ratio
              << " activation_type=" << (int)qwen3_config.activation_type;
    ...
    int64_t ffn_hidden = static_cast<int64_t>(static_cast<double>(n_embd) * 4.5 * 2.0 / 3.0);

    LOG(INFO) << "DEBUG ffn_hidden = " << ffn_hidden << " (expected 12288)";

    if (tp_rank == 0) {
        LOG(INFO) << "is_first_stage=" << is_first_stage << " is_last_stage=" << is_last_stage
                  << " pp_size=" << pp_size;
        LOG(INFO) << "owned_layers count: " << std::count(owned_layers.begin(), owned_layers.end(), true);
    }
    ...
    LOG(INFO) << ">>> Reading q_norm/k_norm weights";
    if (tp_rank == 0) {
        for (const auto &[key, tensor] : state_dict) {
            if (key.find("q_norm") != std::string::npos || key.find("k_norm") != std::string::npos) {
                auto dims = tensor->Dims();
                std::stringstream ss;
                for (size_t j = 0; j < dims.size(); ++j) { ss << dims[j] << (j < dims.size() - 1 ? ", " : ""); }
                LOG(INFO) << "Found: key=" << key << " dtype=" << static_cast<int>(tensor->Dtype()) << " shape=["
                          << ss.str() << "]";
            }
        }
    }
    ...
    for (int i = 0; i < static_cast<int>(n_layer); ++i) {
        if (owned_layers[i]) {
            ...
            if (tp_rank == 0 && i == 0) {
                // 先确认文件位置
                auto pos_before = ifs.tellg();
                LOG(INFO) << "File position before q_norm read: " << pos_before;

                // 直接读到一个临时 float 数组（CPU memory）
                std::vector<float> cpu_buffer(head_dim);
                ifs.read(reinterpret_cast<char *>(cpu_buffer.data()), head_dim * sizeof(float));

                LOG(INFO) << "CPU buffer q_norm first 5: " << cpu_buffer[0] << " " << cpu_buffer[1] << " "
                          << cpu_buffer[2] << " " << cpu_buffer[3] << " " << cpu_buffer[4];

                // 然后 seek 回去，让后续的正常读取逻辑继续
                ifs.seekg(pos_before);
            }
            ...
            if (tp_rank == 0 && i == 0) {
                std::stringstream ss;
                ss << "q_norm dtype=" << static_cast<int>(q_norm_tensor->Dtype()) << " shape=[";
                auto dims = q_norm_tensor->Dims();
                ...
                ss << "]";
                LOG(INFO) << ss.str();
                auto q_data = static_cast<float *>(q_norm_tensor->DataPtr());
                auto k_data = static_cast<float *>(k_norm_tensor->DataPtr());
                LOG(INFO) << "Layer 0 q_norm first 5: " << q_data[0] << " " << q_data[1] << " " << q_data[2] << " "
                          << q_data[3] << " " << q_data[4];
                LOG(INFO) << "Layer 0 k_norm first 5: " << k_data[0] << " " << k_data[1] << " " << k_data[2] << " "
                          << k_data[3] << " " << k_data[4];
            }
    ...
    LOG(INFO) << ">>> Done q_norm/k_norm weights";
