...
void Train(const nn::parallel::Rank &rank) {
    ...
    const auto tokens_per_fwdbwd = FLAGS_batch_size * FLAGS_sequence_length * 1;
    ...
    if (ddp_world_size > 1 && pp_world_size == 1) {
    ...
    if (FLAGS_pipeline_parallel) {
        ...
        model = std::make_shared<nn::parallel::PipelineParallel>(model, FLAGS_num_stages, FLAGS_num_microbatches,
                                                                 shapes, FLAGS_learning_rate, pp_rank);
    }
...
int main(int argc, char *argv[]) {
    ...
    nn::parallel::global::InitAllEnv(FLAGS_nthread_per_process, FLAGS_num_stages, FLAGS_tensor_parallel,
